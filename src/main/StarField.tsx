...
const Section = styled.div`
  width: 100%;
  height: 100vh;
  scroll-snap-align: start; /* 각 섹션이 시작 위치에 스냅되도록 설정 */
...
const Button = styled.button`
  ...
  border: none;
  border-radius: 30px; // 타원형 효과를 주기 위해 border-radius를 증가시킴

  // 커서를 버튼 위에 올렸을 때의 스타일
  cursor: pointer;
  &:hover {
    background-color: #ddd; // 버튼 위에 마우스를 올렸을 때 배경 색상 변경
  }
`;
...
interface ShootingStarsProps {
  ...
  const ShootingStars = styled.div<ShootingStarsProps>`
    z-index: 10;
    width: 5px;
    height: 85px;
...
const Moon = styled.div`
  ...
  box-shadow: 0 0 30px 15px rgba(255, 255, 255, 0.5); /* 빛나는 효과 추가 */
  animation: ${glow} 3s infinite; /* 반짝이는 애니메이션 추가 */
...
const StarField: React.FC = () => {
  ...
  useEffect(() => {
    const generateShootingStars = () => {
      const numberOfStars = Math.random() < 0.5 ? 1 : 2; // 무작위로 1 또는 2개의 유성 생성
      const newShootingStars = Array.from({ length: numberOfStars }, (_, index) => (
        <ShootingStars 
          key={index} 
          top={Math.random() * 100} // 무작위 top 위치 설정
          left={Math.random() * starFieldWidth} // 무작위 left 위치 설정
        />
      ));
      setShootingStars(newShootingStars);
    };

    const interval = setInterval(generateShootingStars, 3000); // 3초 간격으로 유성 생성
  ...
  return (
    <MainContainer>
      <SectionOne>
        <Image src="/persona.png" alt="Persona" />
        <Moon />
        <OwnglyphFont>
          안녕하세요! 저는 Persona입니다
          <br/>
